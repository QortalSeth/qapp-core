...
```bash
npm run build          # Full build: types → i18n → tsup bundle
npm run build:types    # TypeScript declarations only
npm run generate-i18n  # Compile locale JSON into src/i18n/compiled-i18n.json
...
```bash
npx vitest run src/utils/base64.test.ts
```
